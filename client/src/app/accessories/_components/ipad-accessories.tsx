    ...
    <div className="2xl:w-3/5 w-[70%] m-auto">
      <Carousel className="w-full h-full">
        <CarouselContent className="h-full">
          {IpadAccessories.map((value, index) => (
            <CarouselItem key={index} className="md:basis-1/3">
              <div className="bg-slate-100 w-full h-full py-4 text-black rounded-xl flex flex-col justify-between items-center" key={index}>
                ...
                <p className="text-center md:h-20 h-12 2xl:text-lg text-sm font-semibold 2xl:px-8 px-4">{value.name}</p>
                <p className="2xl:text-sm md:text-xs text-base font-light">{formatNumber(parseInt(value.price))}đ</p>
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
